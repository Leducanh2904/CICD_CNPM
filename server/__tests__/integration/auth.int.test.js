...
/**
 * Mock middleware verifyToken để luôn “đăng nhập”.
 * Trả về trực tiếp 1 HÀM (function module) và gán thêm các thuộc tính
 * để tương thích cả require kiểu default lẫn destructuring.
 */
jest.doMock("../../middleware/verifyToken", () => {
  ...
  // tương thích mọi kiểu import/require
  fn.verifyToken = fn;  // const { verifyToken } = require(...)
  fn.default = fn;      // import verifyToken from ...
  fn.__esModule = true;
  return fn;            // <-- TRẢ VỀ CHÍNH HÀM, KHÔNG PHẢI OBJECT
});
...
let app;
beforeAll(() => {
  jest.resetModules(); // clear cache để app dùng mocks ở trên
  app = require("../../app");
});
